...
namespace Progetto
{
    ...
    {
        public ReportViewModel(ObservableCollection<GpxPoint> mainRoute)
        {
            _segmentsCollection = new ObservableCollection<ReportViewModel>();
        ...
        private ObservableCollection<ReportViewModel> _segmentsCollection;

        public ObservableCollection<ReportViewModel> SegmentsCollection
        {
        ...
        public void CalculateStationaryPoints(ReportViewModel currentViewModel)
        {
        ...
        public void GetSubroutes(ReportViewModel currentViewModel)
        {
            ...
            foreach (var point in currentViewModel.MainRoute ?? Enumerable.Empty<GpxPoint>())
            {
                ...
                else if (!alreadyAdded)
                {
                    if (subRoute.Count > 2)
                    {
                        currentViewModel.SegmentsCollection.Add(new ReportViewModel(subRoute) { Nome = $"Sotto-route {index}" });
                        index++;
